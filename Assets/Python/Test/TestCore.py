...
class TestInfos(TestCase):
	...
	def test_leader_player(self):
		# given
		player = gc.getPlayer(0)
		expected_leaderheadinfo = gc.getLeaderHeadInfo(iPacal)
...
class TestPlayers(TestCase):
	...
	def test_contains_civ_id(self):
		self.assert_(iMaya in self.players)
	...
	def test_string(self):
		expected_names = ','.join(['Maya', 'Zapotec', 'Teotihuacan'])
	...
	def test_as_civs(self):
		expected_civs = [iMaya, iZapotec, iTeotihuacan]
	...
	def test_can_be_built_from_civs(self):
		players = Players([iMaya, iZapotec])
...
class TestUnits(TestCase):
	...
	def test_string(self):
		# #MacAurther: player 7 = iNorse, adjective = "Norse"
		expected_string = "Settler (Norse) at (0, 0), Settler (Norse) at (0, 1), Settler (Norse) at (0, 2), Settler (Norse) at (0, 3)"
	...
	def test_owner_civ(self):
		# #MacAurther: player 8 = iEngland
		# given
		indian_unit = gc.getPlayer(8).initUnit(4, 0, 4, UnitAITypes.NO_UNITAI, DirectionTypes.DIRECTION_SOUTH)
		units = self.units + Units([indian_unit])

		# when
		indian_units = units.owner(iEngland)
	...
	def test_domain(self):
		unit = gc.getPlayer(7).initUnit(iCanoe, 0, 1, UnitAITypes.NO_UNITAI, DirectionTypes.DIRECTION_SOUTH)
...
class TestUnitFactory(TestCase):
	...
	def test_owner_civ(self):
		# #MacAurther: player 7 = iNorse
		# when
		greek_units = self.factory.owner(iNorse)
...
class TestPlots(TestCase):
	...
	def test_notowner_civ(self):
		...
		# when
		plots = self.plots.notowner(iMaya)
	...
	def test_land(self):
		# #MacAurther: (32-34, 85-87) is inside iMississippi birth area ((31,84),(36,88)), guaranteed land
		plots = Plots([(32+x, 85+y) for x in range(3) for y in range(3)])
	...
	def test_core_player(self):
		# #MacAurther: Maya core (32,64)-(36,66); (33-35,64-66) avoids the two exceptions at (32,64) and (32,66)
		plots = Plots([(33+x, 64+y) for x in range(3) for y in range(3)])
	...
	def test_core_civ(self):
		# #MacAurther: Maya core (32,64)-(36,66); (33-35,64-66) avoids the two exceptions at (32,64) and (32,66)
		plots = Plots([(33+x, 64+y) for x in range(3) for y in range(3)])
		self.assertEqual(len(plots.core(iMaya)), 9)
	
	def test_no_core_civ(self):
		self.assertEqual(len(self.plots.core(iMaya)), 0)
	...
	def test_area(self):
		# #MacAurther: (54, 67) = Haiti/Hispaniola -- Caribbean island, distinct area from continental Americas
		africaTile = (54, 67)
		americaTile = (27, 47)
	...
	def test_areas(self):
		...
		africaTile = (54, 67)
		americaTile = (27, 47)
		# #MacAurther: (14, 48) = Hilo, Hawaii -- Pacific island, third distinct area
		australiaTile = (14, 48)
...
class TestPlotFactory(TestCase):
	...
	def test_region(self):
		# #MacAurther: TODO - update expected_tiles to actual rHawaii tile list (run game to dump region)
		expected_tiles = [(53, 67), (54, 67), (52, 66), (53, 66), (54, 66), (53, 65), (54, 65), (52, 64), (53, 64)]
		region = Plots(expected_tiles)
		plots = self.factory.region(rHawaii)
	...
	def test_regions_multiple(self):
		# #MacAurther: using two valid DOCAmerica regions to test the regions() API
		self.assert_(self.factory.regions(rYucatan, rMesoamerica).same(self.factory.region(rYucatan) + self.factory.region(rMesoamerica)))
	...
	def test_surrounding_wrap(self):
		expected_tiles = [(0, 0), (1, 0), (0, 1), (1, 1), (82, 0), (82, 1)]
	...
	def test_owner_civ(self):
		...
		expected_tiles = [(0, 0), (1, 0)]
		plots = self.factory.owner(iMaya)
	...
	def test_birth(self):
		plots = self.factory.birth(iMaya)
		assertType(self, plots, Plots)
	...
	def test_core(self):
		plots = self.factory.core(iMaya)
		assertType(self, plots, Plots)
	
	def test_core_period(self):
		# #MacAurther: Cherokee base core (37,80)-(40,85); reservation period (23,82)-(29,85); (27,83) in reservation only
		self.assertEqual(period(iCherokee), -1)
		self.assertEqual((27, 83) in self.factory.core(iCherokee), False)
		self.assertEqual((27, 83) in self.factory.core(iCherokee, iCherokeeReservation), True)
	
	def test_capital(self):
		plot = self.factory.capital(iMaya)
		assertType(self, plot, CyPlot)
		
	def test_newCapital(self):
		plot = self.factory.newCapital(iMaya)
		assertType(self, plot, CyPlot)
		
	def test_respawnCapital(self):
		plot = self.factory.respawnCapital(iMaya)
		assertType(self, plot, CyPlot)
...
class TestCities(TestCase):
	...
	def test_owner_civ(self):
		# #MacAurther: player 7 = iNorse
		cities = self.cities.owner(iNorse)
		expected_tiles = [(0, 0), (0, 2)]
	...
	def test_core_player(self):
		# #MacAurther: (36, 64) is inside Maya's core (32,64)-(36,66)
		city = gc.getPlayer(0).initCity(36, 64)
		cities = self.cities.including(city)
		self.assertEqual(len(cities), 4)

		actual_cities = cities.core(0)
		expected_tiles = [(36, 64)]
	...
	def test_core_civ(self):
		...
		actual_cities = cities.core(iMaya)
		expected_tiles = [(36, 64)]
	...
	def test_coastal(self):
		# #MacAurther: (14, 48) = Hilo, Hawaii -- island tile guaranteed adjacent to ocean
		city = gc.getPlayer(0).initCity(14, 48)
		cities = self.cities.including(city)
		actual_cities = cities.coastal()
		expected_tiles = [(14, 48)]
	...
	def test_notowner_civ(self):
		...
		cities = self.cities.notowner(iNorse)
		actual_tiles = [(city.getX(), city.getY()) for city in cities]
	...
	def test_ever_owned_current(self):
		...
		self.assertEqual(city.getCivilizationType(), iNorse)
		self.assertEqual(self.cities.ever_owned(iNorse), True)
	...
	def test_not_ever_owned(self):
		...
		self.assertEqual(city.getCivilizationType(), iNorse)
		self.assertEqual(self.cities.ever_owned(iMaya), False)
	...
	def test_ever_owned_previous(self):
		...
		try:
			self.assertEqual(city.getPreviousCiv(), iMaya)
			self.assertEqual(city.getCivilizationType(), iNorse)
			self.assertEqual(cities.ever_owned(iMaya), True)
			self.assertEqual(cities.ever_owned(iNorse), True)
		finally:
			city.kill()

	def test_ever_owned_multiple(self):
		# #MacAurther: player 7 = iNorse, player 8 = iEngland
		self.assertEqual(self.cities.ever_owned(iNorse, iEngland), True)

	def test_not_ever_owned_multiple(self):
		self.assertEqual(self.cities.ever_owned(iMaya, iZapotec), False)
...
class TestCityFactory(TestCase):
	...
	def test_owner_civ(self):
		# #MacAurther: player 7 = iNorse
		cities = self.factory.owner(iNorse)
		expected_tiles = [(0, 0), (0, 2)]
...
class TestScenarioUtils(TestCase):
	...
	def test_scenario(self):
		self.assertEqual(scenario(), i1AD)

	def test_scenario_start_turn(self):
		self.assertEqual(scenarioStartTurn(), 0)

	def test_scenario_start_year(self):
		self.assertEqual(scenarioStartYear(), 1)
		
		
class TestUniqueUnitsAndBuildings(TestCase):

	def test_base_building_for_base(self):
		self.assertEqual(base_building(iArena), iArena)
		
	def test_base_building_for_unique(self):
		self.assertEqual(base_building(iBallCourt), iArena)
		
	def test_base_building_for_none(self):
		self.assertEqual(base_building(None), None)
		
	def test_unique_building_for_base(self):
		self.assertEqual(unique_building(0, iArena), iBallCourt)
		
	def test_unique_building_for_unique(self):
		self.assertEqual(unique_building(0, iBallCourt), iBallCourt)
		
	def test_unique_building_no_player(self):
		self.assertEqual(unique_building(-1, iBallCourt), iArena)
		
	def test_unique_building_for_base_civ(self):
		self.assertEqual(unique_building(iMaya, iArena), iBallCourt)
	
	def test_unique_building_for_unique_civ(self):
		self.assertEqual(unique_building(iMaya, iBallCourt), iBallCourt)
		
	def test_unique_building_no_civ(self):
		self.assertEqual(unique_building(NoCiv, iBallCourt), iArena)
		
	def test_base_unit_for_base(self):
		self.assertEqual(base_unit(iSkirmisher), iSkirmisher)
		
	def test_base_unit_for_unique(self):
		self.assertEqual(base_unit(iHolkan), iSkirmisher)
		
	def test_unique_unit_for_base(self):
		self.assertEqual(unique_unit(0, iSkirmisher), iHolkan)
		
	def test_unique_unit_for_unique(self):
		self.assertEqual(unique_unit(0, iHolkan), iHolkan)
		
	def test_unique_unit_no_player(self):
		self.assertEqual(unique_unit(-1, iHolkan), iSkirmisher)
	
	def test_unique_unit_for_base_civ(self):
		self.assertEqual(unique_unit(iMaya, iSkirmisher), iHolkan)
	
	def test_unique_unit_for_unique_civ(self):
		self.assertEqual(unique_unit(iMaya, iHolkan), iHolkan)
	
	def test_unique_unit_no_civ(self):
		self.assertEqual(unique_unit(NoCiv, iHolkan), iSkirmisher)
...
class TestMakeUnit(TestCase):
	...
	def test_missionary(self):
		self.assertEqual(missionary(iCatholicism), iCatholicMiss)
...
class TestNames(TestCase):
	...
	def test_name(self):
		self.assertEqual(name(0), "Maya")

	def test_name_player(self):
		self.assertEqual(name(gc.getPlayer(0)), "Maya")

	def test_name_none(self):
		self.assertRaises(ValueError, name, None)

	def test_adjective(self):
		self.assertEqual(adjective(0), "Mayan")

	def test_adjective_player(self):
		self.assertEqual(adjective(gc.getPlayer(0)), "Mayan")
	
	def test_adjective_none(self):
		self.assertRaises(ValueError, adjective, None)
		
	def test_full_name(self):
		# #MacAurther: player 5 = iInca; TXT_KEY_CIV_INCA_DESC = "The Inca"
		self.assertEqual(fullname(slot(iInca)), "The Inca")

	def test_full_name_player(self):
		# #MacAurther: player 5 = iInca; TXT_KEY_CIV_INCA_DESC = "The Inca"
		self.assertEqual(fullname(player(iInca)), "The Inca")
...
class TestWrap(TestCase):
	...
	def test_wrap_x(self):
		self.assertEqual(wrap(83, 0), (0, 0))

	def test_wrap_x_negative(self):
		self.assertEqual(wrap(-1, 0), (82, 0))

	def test_wrap_y(self):
		self.assertEqual(wrap(0, 122), (0, 121))
...
class TestPlayer(TestCase):
	...
	def test_civ(self):
		p = gc.getPlayer(0)
		self.assertEqual(player(iMaya).getID(), p.getID())
...
class TestCiv(TestCase):
	...
	def test_id(self):
		self.assertEqual(civ(0), iMaya)
		
	def test_player(self):
		self.assertEqual(civ(gc.getPlayer(0)), iMaya)
		
	def test_team(self):
		self.assertEqual(civ(gc.getTeam(gc.getPlayer(0).getTeam())), iMaya)
		
	def test_civ(self):
		self.assertEqual(civ(iMaya), iMaya)
	
	def test_unit(self):
		unit = gc.getPlayer(0).initUnit(4, 0, 0, UnitAITypes.NO_UNITAI, DirectionTypes.DIRECTION_SOUTH)
		
		try:
			self.assertEqual(civ(unit), iMaya)
		finally:
			unit.kill(False, -1)
	...
	def test_plot(self):
		...
		try:
			self.assertEqual(civ(plot), iMaya)
		finally:
			plot.setOwner(-1)
...
class TestPeriod(TestCase):
	...
	def test_period(self):
		game.setPeriod(iMaya, 123)
		iPeriod = period(iMaya)
		
		try:
			self.assertEqual(iPeriod, 123)
		finally:
			game.setPeriod(iMaya, -1)
	
	def test_period_not_set(self):
		iPeriod = period(iCanada)
...
class TestCivDict(TestCase):
	...
	def test_set_civ(self):
		...
		tempdict[iMexico] = 3
		self.assertEqual(tempdict[iMexico], 3)
	...
	def test_default_civ(self):
		...
		self.assertEqual(tempdict[iMaya], -123)
	...
	def test_contains_civ(self):
		self.assert_(iMaya in self.civdict)
...
class TestCivFactory(TestCase):
	...
	def test_of_one(self):
		civs = self.factory.of(iMaya)
		
		assertType(self, civs, Civilizations)
		self.assertEqual(len(civs), 1)
		
		assertType(self, civs[0], Civ)
		self.assertEqual(civs[0], iMaya)
	
	def test_of_more(self):
		civs = self.factory.of(iMaya, iZapotec, iTeotihuacan)
		
		assertType(self, civs, Civilizations)
		self.assertEqual(len(civs), 3)
		
		for i, iCiv in enumerate([iMaya, iZapotec, iTeotihuacan]):
...
class TestCivFactory(TestCase):
	...
	def test_of_civs(self):
		expected_civs = [iMaya, iZapotec, iTeotihuacan]
		actual_civs = self.factory.of(iMaya, iZapotec, iTeotihuacan)
...
class TestCivilizations(TestCase):
	...
	def setUp(self):
		self.civs = Civilizations([iMaya, iZapotec, iTeotihuacan])
	...
	def test_contains_civ_id(self):
		self.assertEqual(iMaya in self.civs, True)
	...
	def test_string(self):
		# #MacAurther: from DynamicNames XMLs: Maya="The Maya", Zapotec="The Zapotecos", Teotihuacan="The Teotihuacanos"
		expected_names = "The Maya,The Zapotecos,The Teotihuacanos"
		actual_names = str(self.civs)
		self.assertEqual(actual_names, expected_names)
		
	def test_alive(self):
		civs = Civilizations([iMaya, iZapotec, iTeotihuacan, iCanada])
		self.assertEqual(iCanada in civs, True)
		
		civs = civs.alive()
		assertType(self, civs, Civilizations)
		self.assertEqual(iCanada in civs, False)
	
	def test_notalive(self):
		civs = Civilizations([iMaya, iCanada])
		self.assertEqual(iMaya in civs, True)
		
		civs = civs.notalive()
		assertType(self, civs, Civilizations)
		self.assertEqual(iCanada in civs, True)
		self.assertEqual(iMaya in civs, False)
	
	def test_without_civ_id(self):
		civs = self.civs.without(iMaya)
		assertType(self, civs, Civilizations)
		self.assertEqual(len(civs), 2)
		self.assertEqual(iMaya not in civs, True)
	...
	def test_without_list(self):
		civs = self.civs.without([iMaya, iZapotec])
		assertType(self, civs, Civilizations)
		self.assertEqual(len(civs), 1)
		self.assertEqual(iTeotihuacan in civs, True)
		
	def test_without_set(self):
		civs = self.civs.without(set([iMaya, iZapotec]))
		assertType(self, civs, Civilizations)
		self.assertEqual(len(civs), 1)
		self.assertEqual(iTeotihuacan in civs, True)
	...
	def test_can_be_built_from_players(self):
		...
		self.assertEqual(iMaya in civs, True)
		self.assertEqual(iZapotec in civs, True)

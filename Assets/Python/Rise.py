...
class Birth(object):
	...
	def revealTerritory(self):
		...
		lRegionList = None
		iCiv = civ(self.iPlayer)
		if iCiv in [iInuit]:
			if player(self.iPlayer).isHuman():
				lRegionList = lInuitRevealedTiles
			else:
				lRegionList = lInuitRevealedTilesAI
		if iCiv in [iEngland, iFrance, iNetherlands, iRussia]:
			lRegionList = lEuropeanRevealed1600AD
		elif iCiv in [iAmerica, iHaiti, iArgentina, iMexico, iColombia, iPeru, iBrazil, iVenezuela]:
			lRegionList = lEuropeanRevealed1750AD
		elif iCiv in [iCSA, iCanada]:

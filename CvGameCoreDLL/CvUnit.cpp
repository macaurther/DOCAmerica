...
void CvUnit::updateCombat(bool bQuick)
{
	...
	if (bFinish)
	{
		...
		else
		{
			...
			gDLL->getInterfaceIFace()->addMessage(getOwnerINLINE(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, pDefender->getDomainType() == DOMAIN_SEA ? "AS2D_OUR_SEA_WITHDRAWL" : "AS2D_OUR_WITHDRAWL", MESSAGE_TYPE_INFO, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_GREEN"), pPlot->getX_INLINE(), pPlot->getY_INLINE());
			szBuffer = gDLL->getText("TXT_KEY_MISC_ENEMY_UNIT_WITHDRAW", getNameKey(), pDefender->getNameKey());
			gDLL->getInterfaceIFace()->addMessage(pDefender->getOwnerINLINE(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, pDefender->getDomainType() == DOMAIN_SEA ? "AS2D_THEIR_SEA_WITHDRAWL" : "AS2D_THEIR_WITHDRAWL", MESSAGE_TYPE_INFO, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), pPlot->getX_INLINE(), pPlot->getY_INLINE());
...
void CvUnit::move(CvPlot* pPlot, bool bShow)
{
	...
	if (!GET_PLAYER(getOwner()).isNative() && pPlot->getTribeStoredUnits() > 0 && GC.getUnitInfo(getUnitType()).isPillage())
	{
		...
		// Clear mission and return to have player reconsider whether to attack or not
		getGroup()->clearMissionQueue();
		return;
	}
...
{
	// MacAurther: Different art styles for different independents
	if (GET_PLAYER(getOwner()).getCivilizationType() == INDEPENDENT2)	// Independent Nations
	{
	...
	else	// Independent Natives
	{
		switch (getOriginalRegion())
		{
			...
			case REGION_NUNAVUT:
			case REGION_GREENLAND:
				return GC.getCivilizationInfo(INUIT).getUnitArtStyleType();
			case REGION_NORTH_CASCADIA:
			case REGION_NORTH_PLAINS:
			case REGION_SOUTH_CASCADIA:
			case REGION_CALIFORNIA:
			case REGION_GREAT_PLAINS:
				return GC.getCivilizationInfo(LAKOTA).getUnitArtStyleType();
			case REGION_ROCKIES:
			case REGION_SOUTHWEST:
			case REGION_BAJA_CALIFORNIA:
			case REGION_SIERRA_MADRES:
			case REGION_TEXAS:
				return GC.getCivilizationInfo(PUEBLO).getUnitArtStyleType();
			case REGION_ONTARIO:
			case REGION_NEW_FOUNDLAND:
			case REGION_NEW_ENGLAND:
			case REGION_MID_ATLANTIC:
			...
			case REGION_COASTAL_PLAIN:
			case REGION_QUEBEC:
				return GC.getCivilizationInfo(HAUDENOSAUNEE).getUnitArtStyleType();
			case REGION_GREAT_LAKES:
			case REGION_APPALACHIA:
			case REGION_DEEP_SOUTH:
			case REGION_FLORIDA:
				return GC.getCivilizationInfo(MISSISSIPPI).getUnitArtStyleType();
			case REGION_CHACO:
			case REGION_CUYO:
			case REGION_PAMPAS:
			case REGION_PATAGONIA:
			...
			case REGION_PARANA:
			case REGION_CARIBBEAN:
				return GC.getCivilizationInfo(ARAWAK).getUnitArtStyleType();
			case REGION_BAJIO:
			case REGION_VERACRUZ:
			case REGION_OAXACA:
			case REGION_YUCATAN:
			...
			case REGION_HAWAII:
				return GC.getCivilizationInfo(HAWAII).getUnitArtStyleType();
		}
	}

...
bool CvCity::canTrain(UnitTypes eUnit, bool bContinue, bool bTestVisible, bool bIgnoreCost, bool bIgnoreUpgrades) const
{
	...
	// Leoreth: can't train slaves -> MacAurther: unless running Slavery or Bondage, and has at least one slave in the city
	if (GC.getUnitInfo(eUnit).isSlave())
	{
		if (!(GET_PLAYER(getOwner()).hasCivic(CIVIC_SLAVERY) || GET_PLAYER(getOwner()).hasCivic(CIVIC_BONDAGE)))
		{
			return false;
		}

		if (getFreeSpecialistCount(SPECIALIST_SLAVE) == 0)
		{
			return false;
		}
	}
...
int CvCity::getCommerceFromPercent(CommerceTypes eIndex, int iYieldRate) const
{
	...
	{
        iCommerce += (iYieldRate - iCommerce - getCommerceFromPercent(COMMERCE_RESEARCH, iYieldRate) - getCommerceFromPercent(COMMERCE_CULTURE, iYieldRate) - getCommerceFromPercent(COMMERCE_ESPIONAGE, iYieldRate) - getCommerceFromPercent(COMMERCE_IMMIGRATION, iYieldRate));	// MacAurther: Immigration
	}

...
{
	return getCivilizationType() == INDIGENOUS;
}
...
bool CvPlayer::canConstruct(BuildingTypes eBuilding, bool bContinue, bool bTestVisible, bool bIgnoreCost, bool bIgnoreTech) const
{
	...
	if (!isHuman())
	{
		...
		else if (isHumanVictoryWonder(eBuilding, (BuildingTypes)BUILDING_ACLLAHUASI, INCA)) return false;
...
void CvPlayer::processBuilding(BuildingTypes eBuilding, int iChange, CvArea* pArea)
{
	...
	// Las Lajas Sanctuary
	if (eBuilding == (BuildingTypes)BUILDING_LAS_LAJAS_SANCTUARY)
	{
		for (pLoopCity = firstCity(&iLoop); pLoopCity != NULL; pLoopCity = nextCity(&iLoop))
		{
...
{
	// Las Lajas Sanctuary
	if (eBuilding == BUILDING_LAS_LAJAS_SANCTUARY)
	{
...
int CvPlayer::unitsRequiredForGoldenAge() const
{
	...
	// Leoreth: Eiffel Tower effect: golden age requires one fewer great person -> MacAurther: Pyramid of the Sun
	if (isHasBuildingEffect((BuildingTypes)BUILDING_PYRAMID_OF_THE_SUN))
	{
...
int CvPlayer::getSpecialistExtraYield(SpecialistTypes eIndex1, YieldTypes eIndex2) const
{
	...
	if (eIndex2 == YIELD_FOOD)
	{
		...
		// for normal specialists extra food is limited to +1	-> MacAurther: Nah, more fun that way

...
int CvPlayerAI::AI_getDifferentReligionAttitude(PlayerTypes ePlayer) const
{
	...
	else if (getCurrentEra() == ERA_COLONIAL)
	{
		iAttitude *= 2;
	}
...
int CvPlayerAI::AI_slaveTradeVal(CvUnit* pUnit) const
{
	...
	bool bOwnerEuropean = (eOwner == SPAIN || eOwner == FRANCE || eOwner == ENGLAND);
	bool bBuyerEuropean = (getID() == SPAIN || getID() == FRANCE || getID() == ENGLAND);

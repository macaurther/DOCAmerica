	...
	for (iPass = 0; iPass < 4; iPass++)
	{
		for(pLoopUnit = firstUnit(&iLoop); pLoopUnit != NULL; pLoopUnit = nextUnit(&iLoop))
		{
			...
			if (bValid)
			{
				...
				if (!bNoDisband)
				{
					if (pLoopUnit->canFight() && !pLoopUnit->isFound()) // Leoreth: do not disband pioneers
					{
						...
						if (pPlotCity != NULL && pPlotCity->getOwnerINLINE() == getID())
						{
							...
							if (iCityExp > 0)
							{
								if ((iExp == 0) || (iExp < (iCityExp + 1) / 2))
								{
									...
									{
										if ((pLoopUnit->getDomainType() != DOMAIN_LAND) || pLoopUnit->plot()->plotCount(PUF_isMilitaryHappiness, getID(), -1, getID()) > 1)
										{
		...
		if (!(pLoopUnit->hasCargo()))
		{
			if (!(pLoopUnit->isGoldenAge()))
			{
				if (pLoopUnit->getUnitInfo().getProductionCost() > 0)
				{
					...
					{
						if (!(pLoopUnit->isMilitaryHappiness()) || !(pLoopUnit->plot()->isCity()) || (pLoopUnit->plot()->plotCount(PUF_isMilitaryHappiness, getID(), -1, getID()) > 2))
						{
...
int CvPlayerAI::AI_slaveTradeVal(CvUnit* pUnit) const
{
	...
	iValue *= 100 + range(iRequiredSlaves - 1, 0, 5) * 10; // MacAurther: Devalue slave because they're more readily available now
	iValue /= 100;
